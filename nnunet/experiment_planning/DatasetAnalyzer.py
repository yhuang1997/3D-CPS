...
class DatasetAnalyzerSSL(DatasetAnalyzer):
    def __init__(self, *args, **kwargs):
        super(DatasetAnalyzerSSL, self).__init__(*args, **kwargs)

    def _get_voxels(self, patient_identifier, modality_id):
        """
        Overwrite by yhuang, get voxels including background & foreground
        ...
        modality = all_data[modality_id]
        mask = np.ones(all_data[-1].shape, dtype=bool)  # crop的时候自动给seg=None的地方生成了mask, 还没看懂,所以就全算上吧
        voxels = list(modality[mask][::10]) # no need to take every voxel
        return voxels
    ...
    def collect_intensity_properties(self, num_modalities):
        if self.overwrite or not isfile(self.intensityproperties_file):
            ...
            for mod_id in range(num_modalities):
                results[mod_id] = OrderedDict()
                # replace get_voxels_from_foreground with get_voxels
