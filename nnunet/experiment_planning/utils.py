...
def create_lists_from_splitted_dataset_json(base_folder_splitted):
    """
    Write by yhuang, labeled & unlabeled database is recorded in dataset.json
    :param base_folder_splitted:
    :return:
    """
    lists = []
    ...
    imagesTrBase = d['imagesTrBase']
    labelsTrBase = d['labelsTrBase']
    uimagesTrBase = d['uimagesTrBase']
    training_files = d['training']
    ...
    for tr in training_files:
        cur_pat = []
        for mod in range(num_modalities):
            cur_pat.append(join(imagesTrBase, tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))
        cur_pat.append(join(labelsTrBase, tr['label'].split("/")[-1]))
        lists.append(cur_pat)

    for un in unlabeled_files:
        cur_pat = []
        for mod in range(num_modalities):
            cur_pat.append(join(uimagesTrBase, un.split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))
        cur_pat.append(None)  # unlabeled set 没有 label
        lists.append(cur_pat)
    return lists, {int(i): d['modality'][str(i)] for i in d['modality'].keys()}
...
def crop_ssl(task_string, override=False, num_threads=default_num_threads):
    """
    Overwrite by yhuang
    :param task_string:
    :param override:
    ...
    splitted_4d_output_dir_task = join(nnUNet_raw_data, task_string)
    # Difference!
    lists, _ = create_lists_from_splitted_dataset_json(splitted_4d_output_dir_task)
