...
from nnunet.experiment_planning.DatasetAnalyzer import DatasetAnalyzer, DatasetAnalyzerSSL
from nnunet.experiment_planning.utils import crop, crop_ssl
from nnunet.paths import *
import shutil
from nnunet.utilities.task_name_id_conversion import convert_id_to_task_name
from nnunet.preprocessing.sanity_checks import verify_dataset_integrity, verify_dataset_integrity_ssl
...
def main():
    ...
    parser.add_argument("-ssl", required=False, default=False, action="store_true",
                        help="Use this to plan & preprocess data for ssl training.")
    ...
    for i in task_ids:
        ...
        if args.verify_dataset_integrity:
            if not ssl:
                ...
                verify_dataset_integrity_ssl(join(nnUNet_raw_data, task_name))
        if not ssl:
            crop(task_name, False, tf)
        else:
            crop_ssl(task_name, False, tf)
    ...
    for t in tasks:
        ...
        if not ssl:
            ...
            dataset_analyzer = DatasetAnalyzerSSL(cropped_out_dir, overwrite=False, num_processes=tf)  # this class creates the fingerprint
            _ = dataset_analyzer.analyze_dataset(collect_intensityproperties)  # this will write output files that will be used by the ExperimentPlanner

        print("Data Analyzing is over!")
