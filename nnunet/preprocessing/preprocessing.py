...
class GenericPreprocessor(object):
    ...
    def resize(self,
               target_shape,
               input_folder_with_preprocessed_npz,
               output_folder,
               resize_fn=None,
               resize_kwargs=None):
        print("Initializing to run preprocessing for resize original images.")
        print("npz folder:", input_folder_with_preprocessed_npz)
        print("output_folder:", output_folder)

        if resize_fn is None:
            resize_fn = resize
        if resize_kwargs is None:
            resize_kwargs = {'order': 3,
                             'mode': 'edge',
                             'anti_aliasing': False
                             }
        list_of_cropped_npz_files = subfiles(input_folder_with_preprocessed_npz, True, None, ".npz", True)
        maybe_mkdir_p(output_folder)

        for j, case in enumerate(list_of_cropped_npz_files):
            case_identifier = get_case_identifier_from_npz(case)
            data, _, _ = self.load_cropped(input_folder_with_preprocessed_npz, case_identifier)
            data = data[0]  # CHWD -> HWD
            resized_data = resize_fn(data, target_shape, **resize_kwargs)
            print("saving: ", os.path.join(output_folder), "%s.npz" % case_identifier)
            np.savez_compressed(os.path.join(output_folder, "%s.npz" % case_identifier),
                                data=resized_data.astype(np.float32))
