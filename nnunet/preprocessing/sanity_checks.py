...
def verify_all_same_orientation_new(*folders):
    """
    Overwrite by yhuang
    This should run after cropping
    :param folder:
    ...
    for folder in folders:
        nii_files.extend(subfiles(folder, suffix=".nii.gz", join=True))
    orientations = []
    for n in nii_files:
        img = nib.load(n)
        affine = img.affine
...
def verify_dataset_integrity_ssl(folder):
    """
    Overwrite the function mainly in two aspects:
    1. Do not copy datasets into nnUNet_raw_base and add dataset_base in dataset.json
    2.
    checks if all training cases and labels are present
    checks if all test cases (if any) are present
    for each case, checks whether all modalities apre present
    ...
    imagesTr_base = dataset["imagesTrBase"]
    labelsTr_base = dataset["labelsTrBase"]
    imagesTs_base = dataset["imagesTsBase"]
    uimagesTr_base = dataset["uimagesTrBase"]

    assert isdir(imagesTr_base), "There needs to be a imagesTr subfolder in folder, folder=%s" % imagesTr_base
    assert isdir(labelsTr_base), "There needs to be a labelsTr subfolder in folder, folder=%s" % labelsTr_base
    assert isdir(imagesTs_base), "There needs to be a imagesTs subfolder in folder, folder=%s" % imagesTs_base
    assert isdir(uimagesTr_base), "There needs to be a uimagesTr subfolder in folder, folder=%s" % uimagesTr_base

    training_cases = dataset['training']
    test_cases = dataset['test']
    unlabeled_cases = dataset['unlabeled']
    num_modalities = len(dataset['modality'].keys())
    expected_train_identifiers = [i['image'].split("/")[-1][:-7] for i in training_cases]
    expected_test_identifiers = [i.split("/")[-1][:-7] for i in test_cases]
    expected_unlabeled_identifiers = [i.split("/")[-1][:-7] for i in unlabeled_cases]

    ## check training set
    nii_files_in_imagesTr = subfiles(imagesTr_base, suffix=".nii.gz", join=False)
    nii_files_in_labelsTr = subfiles(labelsTr_base, suffix=".nii.gz", join=False)
    nii_files_in_uimagesTr = subfiles(uimagesTr_base, suffix=".nii.gz", join=False)
    ...
    # check all cases
    if len(expected_train_identifiers) != len(np.unique(expected_train_identifiers)): raise RuntimeError("found duplicate training cases in dataset.json")
    if len(expected_unlabeled_identifiers) != len(np.unique(expected_unlabeled_identifiers)): raise RuntimeError("found duplicate unlabeled cases in dataset.json")

    print("Verifying training set")
    for c in expected_train_identifiers:
        print("checking case", c)
        # check if all files are present
        expected_label_file = join(labelsTr_base, c + ".nii.gz")
        label_files.append(expected_label_file)
        expected_image_files = [join(imagesTr_base, c + "_%04.0d.nii.gz" % i) for i in range(num_modalities)]
        assert isfile(expected_label_file), "could not find label file for case %s. Expected file: \n%s" % (
            c, expected_label_file)
        assert all([isfile(i) for i in
                    expected_image_files]), "some image files are missing for case %s. Expected files:\n %s" % (
    ...
    print("Verifying unlabeled set")
    for c in expected_unlabeled_identifiers:
        print("checking case", c)
        # check if all files are present
        expected_uimage_files = [join(uimagesTr_base, c + "_%04.0d.nii.gz" % i) for i in range(num_modalities)]
        assert all([isfile(i) for i in
                    expected_uimage_files]), "some image files are missing for case %s. Expected files:\n %s" % (
            c, expected_uimage_files)

        # now remove checked files from the lists nii_files_in_imagesTr and nii_files_in_labelsTr
        for i in expected_uimage_files:
            nii_files_in_uimagesTr.remove(os.path.basename(i))
    ...
    assert len(
        nii_files_in_labelsTr) == 0, "there are training cases in labelsTr that are not listed in dataset.json: %s" % nii_files_in_labelsTr
    assert len(
        nii_files_in_uimagesTr) == 0, "there are training cases in imagesTr that are not listed in dataset.json: %s" % nii_files_in_uimagesTr
    ...
    if len(expected_test_identifiers) > 0:
        print("Verifying test set")
        nii_files_in_imagesTs = subfiles(imagesTs_base, suffix=".nii.gz", join=False)

        for c in expected_test_identifiers:
            # check if all files are present
            expected_image_files = [join(imagesTs_base, c + "_%04.0d.nii.gz" % i) for i in range(num_modalities)]
            assert all([isfile(i) for i in
                        expected_image_files]), "some image files are missing for case %s. Expected files:\n %s" % (
                c, expected_image_files)
    ...
    # all_same, unique_orientations = verify_all_same_orientation(imagesTr_base)
    # Verify orientations for labeled & unlabeled images
    all_same, unique_orientations = verify_all_same_orientation_new(imagesTr_base, uimagesTr_base)
