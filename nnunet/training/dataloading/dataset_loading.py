...
def unpack_dataset(folder, threads=default_num_threads, key="data"):
    ...
    npy_identifiers = [x[:-4] for x in npy_files]
    filted_npz_files = []  # 过滤已经有npy文件的data
    for f in npz_files:
        identifier = f.split('/')[-1].split('.')[0]
        if not identifier in npy_identifiers:
            filted_npz_files.append(f)
    p.map(convert_to_npy, zip(filted_npz_files, [key] * len(filted_npz_files)))
    p.close()
    p.join()
...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # donly = data_only ; segonly = seg_only
...
if __name__ == "__main__":
    ...
    folder_with_preprocessed_data = '/public/datasets/yhuang/nnunet_workdir/nnUNet_preprocessed/Task666_FLARE2022/nnUNetData_plans_v2.1_stage1/'
    folder_with_resized_data = '/public/datasets/yhuang/nnunet_workdir/nnUNet_preprocessed/Task666_FLARE2022/Resize_40_224_192/'
    dataset_directory = '/public/datasets/yhuang/nnunet_workdir/nnUNet_preprocessed/Task666_FLARE2022/'
    plans_file = '/public/datasets/yhuang/nnunet_workdir/nnUNet_preprocessed/Task666_FLARE2022/nnUNetPlansv2.1_plans_3D.pkl'
    ...
    basic_generator_patch_size = None
    ...
    # unpack_dataset
    unpack_dataset(folder_with_preprocessed_data, threads=default_num_threads, key="data")
    ...
    dl_tr = DataLoader3D_GSW(dataset_tr, basic_generator_patch_size, patch_size, batch_size,
                             False, oversample_foreground_percent=oversample_foreground_percent,
                             pad_mode="constant", pad_sides=pad_all_sides, memmap_mode='r',
                             folder_resized_data=folder_with_resized_data)
    dl_val = DataLoader3D_GSW(dataset_val, patch_size, patch_size, batch_size, False,
