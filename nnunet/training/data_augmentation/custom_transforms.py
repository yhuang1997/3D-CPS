...
def convert_3d_to_2d_generator(data_dict):
    ...
    # rewrite by yhuang
    if 'original' in data_dict.keys():
        shp = data_dict['original'].shape
        data_dict['original'] = data_dict['original'].reshape((shp[0], shp[1] * shp[2], shp[3], shp[4]))
        data_dict['orig_shape_original'] = shp
    return data_dict
...
def convert_2d_to_3d_generator(data_dict):
    ...
    # rewrite by yhuang
    if 'original' in data_dict.keys():
        shp = data_dict['orig_shape_original']
        current_shape = data_dict['original'].shape
        data_dict['original'] = data_dict['original'].reshape((shp[0], shp[1], shp[2], current_shape[-2], current_shape[-1]))
    return data_dict
