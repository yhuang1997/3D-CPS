...
def convert_npz_to_nifti(input_filename: str, output_filename_truncated: str):
    """
    cropped data and seg saved as .npz
    :param input_filename:
    :param output_filename_truncated:
    :return:
    """
    data = np.load(input_filename)["data"]
    img = sitk.GetImageFromArray(data[0])
    seg = sitk.GetImageFromArray(data[1])
    sitk.WriteImage(img, output_filename_truncated + "_0000.nii.gz")
    sitk.WriteImage(seg, output_filename_truncated + ".nii.gz")


if __name__ == '__main__':
    # files = glob.glob("/public/datasets/yhuang/nnunet_workdir/nnUNet_raw_data_base/nnUNet_cropped_data/Task666_FLARE2022/*.npz")
    files = glob.glob("/public/datasets/yhuang/nnunet_workdir/nnUNet_raw_data_base/nnUNet_cropped_data/Task667_FLARE2022CPS/*.npz")
    for f in files:
        convert_npz_to_nifti(f, f.split('.')[0])